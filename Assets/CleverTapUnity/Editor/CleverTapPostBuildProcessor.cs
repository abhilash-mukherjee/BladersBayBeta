...
public class CleverTapPostBuildProcessor : IPreprocessBuildWithReport
{
    ...
    private static void SetClevelTapIds()
    {
		JMRAnalyticsManager manager = GameObject.FindObjectOfType<JMRAnalyticsManager>();
		if(manager == null)
			return;

		CleverTapEnvDetails envDetails = manager.GetAnalyticsEnvData.CleverTap;

		if (!envDetails.isInUse)
			return;

		CLEVERTAP_ACCOUNT_ID = envDetails.accountId;
		CLEVERTAP_ACCOUNT_TOKEN = envDetails.accountAuthToken;
		CLEVERTAP_ACCOUNT_REGION = envDetails.accountRegion;
    }
